...
namespace DuiLib {
	...
	{
		// 1、aaa.jpg
		// 2、file='aaa.jpg' res='' restype='0' dest='0,0,0,0' source='0,0,0,0' corner='0,0,0,0' pos='center center' 
		...
		for( int i = 0; i < 2; ++i ) {
			...
			while( *pStrImage != _T('\0') ) {
				...
				if( !sValue.IsEmpty() ) {
					...
					else if ( sItem == _T("imgpos"))
					{
						if (_tcsicmp(sValue.GetData(), _T("lefttop")) == 0){
							imgPos = IMG_POS_LEFT_TOP;
						}
						else if (_tcsicmp(sValue.GetData(), _T("centertop")) == 0){
							imgPos = IMG_POS_CENTER_TOP;
						}
						else if (_tcsicmp(sValue.GetData(), _T("centertop")) == 0){
							imgPos = IMG_POS_CENTER_TOP;
						}
						else if (_tcsicmp(sValue.GetData(), _T("righttop")) == 0)
						{
							imgPos = IMG_POS_RIGHT_TOP;
						}
						else if (_tcsicmp(sValue.GetData(), _T("leftmid")) == 0){
							imgPos = IMG_POS_LEFT_MID;
						}
						else if (_tcsicmp(sValue.GetData(), _T("centermid")) == 0){
							imgPos = IMG_POS_CENTER_MID;
						}
						else if (_tcsicmp(sValue.GetData(), _T("rightmid")) == 0){
							imgPos = IMG_POS_RIGHT_MID;
						}
						else if (_tcsicmp(sValue.GetData(), _T("leftbottom")) == 0){
							imgPos = IMG_POS_LEFT_BOTTOM;
						}
						else if (_tcsicmp(sValue.GetData(), _T("centerbottom")) == 0){
							imgPos = IMG_POS_CENTER_BOTTOM;
						}
						else if (_tcsicmp(sValue.GetData(), _T("rightbottom")) == 0){
							imgPos = IMG_POS_RIGHT_BOTTOM;
						}
					}
				}
				if( *pStrImage++ != _T(' ') ) break;
